...
def test_get_customers_multiple(client, session):
    ...
    """
    customer1 = Customer(name="Customer One", company_id=1, contact_person="John Doe")
...
def test_patch_customer_name_too_long(client, session, customer):
    response = client.patch(f"/customers/{customer.id}", json={"name": "a" * 101})
    assert response.status_code == 400
    data = response.get_json()
    assert "name" in data["error"]
...
def test_get_by_company_id_sqlalchemy_error(client, monkeypatch):
    ...
    monkeypatch.setattr(type(Customer.query), "filter_by", lambda *a, **k: FakeQuery())
