...
def test_jwt_cookie_forwarding_to_guardian(client, session):
    """
    ...
    with mock.patch('requests.get', return_value=mock_response) as mock_get:
        with mock.patch.dict('os.environ', {'GUARDIAN_SERVICE_URL': 'http://guardian:5000'}):
            ...
            print(f"✅ JWT token successfully forwarded to Guardian: {cookie_header[:50]}...")


def test_post_role_jwt_cookie_forwarding(client, session):
    """
    ...
    with mock.patch('requests.post', return_value=mock_response) as mock_post:
        with mock.patch.dict('os.environ', {'GUARDIAN_SERVICE_URL': 'http://guardian:5000'}):
            ...
            # Verify the cookie contains our JWT token
            assert f'access_token={jwt_token}' in cookie_header
            print(f"✅ JWT token successfully forwarded to Guardian in POST: {cookie_header[:50]}...")


def test_individual_role_jwt_forwarding(client, session):
    """
    ...
    with mock.patch('requests.get', return_value=mock_get_response) as mock_get_del:
        with mock.patch('requests.delete', return_value=mock_delete_response) as mock_delete:
            with mock.patch.dict('os.environ', {'GUARDIAN_SERVICE_URL': 'http://guardian:5000'}):
                ...
                print("✅ JWT forwarded in individual role DELETE (both GET verification and DELETE calls)")