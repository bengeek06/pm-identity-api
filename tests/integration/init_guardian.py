...
def init_guardian_db():
    ...
    try:
        ...
        if response.status_code in [200, 409]:  # 200=initialized, 409=already done
            print(f"✓ Guardian /init-db: {response.json().get('message', 'OK')}")
            return True
        else:
            print(f"✗ Guardian /init-db failed: {response.status_code} - {response.text}")
            return False
...
def create_admin_policy(company_id, token):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"✗ Failed to create admin policy: {response.status_code} - {response.text}")
            return None
    except requests.exceptions.RequestException as e:
...
def associate_permissions_to_policy(policy_id, permissions, token):
    ...
    print(f"✓ Associated {success_count}/{len(permissions)} permissions to policy")
...
def associate_policy_to_role(role_id, policy_id, token):
    ...
    try:
        ...
        if response.status_code in [201, 409]:
            print(f"✓ Associated policy to role")
            return True
        else:
...
def main(company_id, token):
    ...
    print(f"✅ Guardian initialized successfully!")
