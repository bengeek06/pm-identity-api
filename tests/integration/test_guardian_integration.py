...
def is_guardian_available():
    ...
    guardian_url = os.environ.get("GUARDIAN_SERVICE_URL", "http://guardian:8000")
    try:
...
def test_guardian_user_roles_integration(integration_client, real_user, integration_token):
    """
    ...
    print(f"✅ Guardian integration OK - User {user_id} has {len(data['roles'])} roles")


@pytest.mark.integration  
def test_guardian_user_permissions_integration(integration_client, real_user, integration_token):
    """
    ...
    print(f"✅ Guardian permissions integration OK - {len(data['permissions'])} permissions")


@pytest.mark.integration
def test_guardian_access_control_integration(integration_client, real_user, integration_token):
    """
    ...
    if response.status_code == 403:
        data = response.get_json()
        assert "access" in str(data).lower() or "permission" in str(data).lower()
