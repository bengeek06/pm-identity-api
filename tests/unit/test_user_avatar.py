...
def test_upload_avatar_no_file_provided(client, session):
...
def test_upload_avatar_storage_service_disabled(client, app, session):
...
def test_upload_avatar_success(mock_upload, client, app, session):
...
@mock.patch("app.resources.user_avatar.upload_avatar_via_proxy")
def test_upload_avatar_validation_error(mock_upload, client, app, session):
...
@mock.patch("app.resources.user_avatar.upload_avatar_via_proxy")
def test_upload_avatar_storage_service_error(mock_upload, client, app, session):
...
def test_get_avatar_user_not_found(client, app):
...
def test_get_avatar_user_has_no_avatar(client, session):
...
def test_get_avatar_success(mock_get, client, app, session):
    ...
    mock_response.iter_content = mock.Mock(return_value=[b"fake", b"image", b"data"])
...
def test_delete_avatar_user_not_found(client, app):
...
def test_delete_avatar_user_has_no_avatar(client, session):
...
def test_delete_avatar_success(mock_delete, client, app, session):
    ...
    with mock.patch("requests.post", return_value=mock_check_access):
        ...
        mock_delete.assert_called_once_with(user_id, company_id, "test-file-id")
...
@mock.patch("app.resources.user_avatar.delete_avatar")
def test_delete_avatar_continues_on_storage_error(mock_delete, client, app, session):
