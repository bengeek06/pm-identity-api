...
class UserAvatarResource(Resource):
    ...
    def get(self, user_id):
        ...
        storage_service_url = os.environ.get("STORAGE_SERVICE_URL", "http://storage-service:5000")
        timeout = int(os.environ.get("STORAGE_REQUEST_TIMEOUT", "30"))
        
        try:
            logger.debug(f"Fetching avatar from Storage Service: bucket_type={bucket_type}, bucket_id={bucket_id}, logical_path={logical_path}")
            ...
            if response.status_code != 200:
                logger.error(f"Storage Service returned {response.status_code}: {response.text}")
